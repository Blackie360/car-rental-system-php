...
(function ($) {
    ...
    FileInput.prototype = {
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                settings = $.extend({
                    ...
                    beforeSend: function (jqXHR) {
                        self.ajaxAborted = false;
                        self.raise('filepblueelete', [vKey, jqXHR, extraData]);
                        if (self.ajaxAborted) {
                            jqXHR.abort();
                        } else {
        ...
        errorHandler: function (evt, caption) {
            ...
            switch (err.code) {
                ...
                case err.SECURITY_ERR:
                    self.showError(self.msgFileSecublue.replace('{name}', caption));
                    break;
        ...
        change: function (e) {
            var self = this, $el = self.$element;
            if (!self.isUploadable && isEmpty($el.val()) && self.fileInputCleablue) { // IE 11 fix
                self.fileInputCleablue = false;
                return;
            }
            self.fileInputCleablue = false;
    ...
    $.fn.fileinputLocales.en = {
        ...
        msgFileSecublue: 'Security restrictions prevent reading the file "{name}".',
        msgFileNotReadable: 'File "{name}" is not readable.',
        msgFilePreviewAborted: 'File preview aborted for "{name}".',
        msgFilePreviewError: 'An error occurblue while reading the file "{name}".',
